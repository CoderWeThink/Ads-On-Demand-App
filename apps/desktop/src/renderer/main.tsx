  ...
  return <main className="welcome">
    ...
    <section className="form-area">
      ...
      <div id="account-panel" role="tabpanel" aria-labelledby={create ? 'create-tab' : 'signin-tab'}>
        <form key={mode} onSubmit={event => { event.preventDefault(); unavailable(create ? 'Account creation' : 'Sign-in'); }}>
          ...
          <div className="password-input"><input id="password" type={visible ? 'text' : 'password'} placeholder={create ? 'At least 12 characters' : 'Enter your password'} minLength={create ? 12 : 1} autoComplete="off" required/><button type="button" aria-label={visible ? 'Hide password' : 'Show password'} onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div>
        ...
        <p className="availability-note">Account services are coming soon.</p>
